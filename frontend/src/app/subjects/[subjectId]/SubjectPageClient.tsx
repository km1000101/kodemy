...
export function SubjectPageClient({ subjectId }: { subjectId: string }) {
  ...
  const numericSubjectId = useMemo(() => Number(subjectId), [subjectId]);
  ...
  useEffect(() => {
    ...
    apiGet<{ tree: Tree }>(`/api/subjects/${numericSubjectId}/tree`)
      .then((data) => setTree(data.tree))
      .catch((err) => {
        if (err instanceof ApiError && err.status === 401) {
          router.push(`/auth/login?next=${encodeURIComponent(`/subjects/${numericSubjectId}`)}`);
          return;
        }
        setError("Failed to load curriculum. Try logging in again.");
      })
      .finally(() => setLoading(false));
  }, [numericSubjectId, router]);
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[360px,1fr] gap-5">
                  ...
                  <div className="space-y-1">
                    {section.videos.map((v) => {
                      ...
                      return (
                        <Link
                          key={v.id}
                          href={`/subjects/${numericSubjectId}/video/${v.id}`}
                          className={[
                          ...
                          <div className="flex items-center justify-between gap-3">
                            ...
                            <div
                              className={`text-xs font-medium ${
                                v.isCompleted ? "text-[#a435f0]" : "text-zinc-500"
                              }`}
                            >
          ...
          <section className="rounded-xl border border-zinc-200 bg-white shadow-sm p-6 min-h-[520px]">
            <div className="text-sm text-zinc-600">Select a lesson from the sidebar to start learning.</div>
            <div className="mt-6 rounded-2xl border border-zinc-200 bg-zinc-50 p-5">
              <div className="text-sm font-semibold text-zinc-950">How unlocking works</div>
              <p className="mt-2 text-sm text-zinc-600 leading-relaxed">
                Each video is locked until you complete the previous one in the global order (sections sorted, then
                videos sorted). Your watch progress is stored per video.
              </p>
