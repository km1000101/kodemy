...
export default function VideoPage({
  ...
  useEffect(() => {
    ...
    Promise.resolve(params)
      .then((p) => {
        if (cancelled) return;
        setSubjectId(Number(p.subjectId));
        setVideoId(Number(p.videoId));
      })
      .catch(() => {
        if (cancelled) return;
        setSubjectId(null);
  ...
  async function sendProgress(seconds: number, isCompleted: boolean) {
    ...
    await apiPost(`/api/progress/videos/${videoId}`, {
  ...
  useEffect(() => {
    if (subjectId == null || videoId == null) return;

    apiGet<{ video: Video }>(`/api/videos/${videoId}`)
      .then((data) => setVideo(data.video))
      .catch((err) => {
        if (err instanceof ApiError && err.status === 401) {
          router.push(
            `/auth/login?next=${encodeURIComponent(`/subjects/${subjectId}/video/${videoId}`)}`
          );
          return;
        }
  ...
  }, [videoId, router, subjectId]);

  useEffect(() => {
    return () => {
      if (intervalRef.current) window.clearInterval(intervalRef.current);
      intervalRef.current = null;
      // Cleanup is best-effort; if playerRef isn't ready yet, skip.
  ...
  if (subjectId == null || videoId == null) {
    return (
        ...
        <main className="rounded-xl border border-zinc-200 bg-white shadow-sm p-6 min-h-[520px]">
          <div className="flex items-start justify-between gap-3 flex-wrap">
            ...
            <div className="text-xs text-zinc-500">
              {video.durationSeconds ? `${video.durationSeconds}s` : "Duration unknown"}
            </div>
          </div>
              ...
              <div className="relative rounded-xl overflow-hidden border border-zinc-200">
                <YouTube
                  ...
                  onReady={(event) => {
                    ...
                    // Best-effort: send current position once player is ready.
